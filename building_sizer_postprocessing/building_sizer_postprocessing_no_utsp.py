...
def collect_building_sizer_results_and_find_kpi_winners(files: List[Path], kpi_column: str, find_min_or_max: str, parent_folder: Path,):
    ...
    for file_path in files:
        ...
        selected_row = get_min_or_max_row_for_column(df=df, file_path=file_path, column=kpi_column, find_min_or_max=find_min_or_max)
    ...
    # save as csv
    save_minima_or_maxima_to_csv(df=df_selected_rows, column=kpi_column[1], output_dir=parent_folder, find_min_or_max=find_min_or_max)
...
def find_kpi_winners_for_all_buildings(
    ...
    for kpi_column in kpi_columns:
        collect_building_sizer_results_and_find_kpi_winners(files=files, kpi_column=kpi_column, find_min_or_max=find_min_or_max, parent_folder=parent_folder)
        

 # --------------------------------

def collect_all_bs_results(
    parent_folder: Path,
    kpi_columns: List[str],
    filename: str = "building_sizer_results.csv",):
    files = go_through_parent_folder_and_find_all_building_sizer_results(
    ...
    for kpi_column in kpi_columns:
        df_collected = collect_building_sizer_results(files=files, kpi_column=kpi_column, parent_folder=parent_folder)
        filter_by_building_type(df=df_collected, parent_folder=parent_folder, kpi_column=kpi_column)

def filter_by_building_type(df: pd.DataFrame, parent_folder: Path, kpi_column: str, building_code_column: str = ("Input", "building_code"), energy_system_column: tuple = ("Input", "energy_system_combination")):
    ...
    for b_type in buildig_types:
        ...
        filtered_sorted_df.to_csv(parent_folder / f"filtered_df_{b_type}.csv", index=False)
        plot_ratings_of_each_energy_system_config_as_scatterplot(df=filtered_sorted_df, output_dir=parent_folder, kpi_column=kpi_column, building_type=b_type, hue_column=building_code_column, energy_system_column=energy_system_column)

def sort_df_according_to_one_building(df: pd.DataFrame, kpi_column: str, energy_system_column: str, reference_house: str, building_code_column: str = ("Input", "building_code")):
    ...
    sorted_reference_systems = (
        reference_df.sort_values(by=kpi_column)[energy_system_column].tolist()
    )
    ...
    if missing:
        print("Reference house is missing these systems:", missing)
    return df
...
def plot_ratings_of_each_energy_system_config_as_scatterplot(
    ...
    # Save figure
    filepath = output_dir / f"{kpi_column[1]}_per_energy_system_{building_type}.png"
    plt.savefig(filepath, dpi=300, bbox_inches='tight')
    print("Made plot for", kpi_column[1], building_type)
...
def collect_building_sizer_results(files: List[Path], parent_folder: Path, kpi_column: str, building_code_column: str = ("Input", "building_code"), building_id_column: str = ("Input", "building_id"), energy_system_column: str = ("Input", "energy_system_combination")):
    collected_dfs = []
    for file_path in files:
        # read each buildig sizer result
        df = read_building_sizer_results(file_path=file_path)
    ...
    return df_collected
    # # save as csv
    # save_collected_results_to_csv(df=df_collected, column=kpi_column[1], output_dir=parent_folder)
...
if __name__ == "__main__":
    ...
    # kpi_config_fields = ["combined_kpi_balanced", "combined_kpi_costs", "combined_kpi_emissions", "combined_kpi_technical"]

    column_of_interest = [("Output", kpi_field) for kpi_field in kpi_config_fields_min]
    COLUMNS_OF_INTEREST = column_of_interest  # Add more as needed

    find_kpi_winners_for_all_buildings(parent_folder=PARENT_FOLDER, kpi_columns=COLUMNS_OF_INTEREST, find_min_or_max=FIND_MIN_OR_MAX)
    ...
    column_of_interest = [("Output", kpi_field) for kpi_field in kpi_config_fields_max]
    COLUMNS_OF_INTEREST = column_of_interest  # Add more as needed
    ...
    collect_all_bs_results(parent_folder=PARENT_FOLDER, kpi_columns=COLUMNS_OF_INTEREST)
